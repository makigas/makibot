...
export default function threadOnlyChannels(makibot: Makibot): express.Router {
  const router = express.Router({ mergeParams: true });

  /* List all the thread channels. */
  router.get("/", async (req: RouterRequest<unknown, { channels: string[] }>, res) => {
    const value: string[] = await res.locals.server.tagbag.tag("threadchannels").get([]);
  ...
  router.post("/", async (req: RouterRequest<{ channel: string }>, res) => {
    ...
    await res.locals.server.tagbag.tag("threadchannels").set(newValue);
    return res.status(202).send("OK");
  });

  router.delete("/:id", async (req: RouterRequest<{ id: string }>, res) => {
    const oldValue: string[] = await res.locals.server.tagbag.tag("threadchannels").get([]);
