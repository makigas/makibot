...
import { Command, CommandGroup, CommandMessage, CommandRegistry } from "discord.js-commando";
...
export = class HelpCommand extends Command {
  ...
  private groupsString(registry: CommandRegistry, includeAdmin: boolean): string {
    let info = (g: CommandGroup) => `__${g.name}__\n${this.commandsString(g)}`;
  ...
  private commandsString(group: CommandGroup): string {
    let info = (c: Command) => `**${c.name}**: ${c.description}`;
