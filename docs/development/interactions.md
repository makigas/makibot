...
Todas las interacciones, sean del tipo que sean, deben implementar una función llamada `handle`, que recibe el tipo de interacción ya filtrado. Por ejemplo, un `CommandInteractionHandler` tiene una función llamada `handle` con un parámetro de tipo `CommandInteraction`. Un `UserContextMenuInteractionHandler` recibe como parámetro para su función `handle` un `UserContextMenuInteraction`.
